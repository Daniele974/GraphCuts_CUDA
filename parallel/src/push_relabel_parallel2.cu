...
int executePushRelabel(std::string filename, std::string output){
    ...
    int n = 0;
    int *capacity, *excess, *height, *residual;
    ...
    */
    ...
    // Mapping memoria host a device
    cudaHostGetDevicePointer(&d_capacity, capacity, 0);
    cudaHostGetDevicePointer(&d_excess, excess, 0);
    cudaHostGetDevicePointer(&d_height, height, 0);
    cudaHostGetDevicePointer(&d_residual, residual, 0);
    ...
    cudaFreeHost(capacity);
    cudaFreeHost(excess);
    cudaFreeHost(height);
    cudaFreeHost(residual);
