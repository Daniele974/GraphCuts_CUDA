./serial/prserial "input_data/graph1.txt" "results/graph1_serial_results.json"
./parallel/prparallel "input_data/graph1.txt" "results/graph1_parallel_results.json"

./serial/prserial "input_data/graph2.txt" "results/graph2_serial_results.json"
./parallel/prparallel "input_data/graph2.txt" "results/graph2_parallel_results.json"

./serial/prserial "input_data/graph3.txt" "results/graph3_serial_results.json"
./parallel/prparallel "input_data/graph3.txt" "results/graph3_parallel_results.json"

./serial/prserial "input_data/graph4.txt" "results/graph4_serial_results.json"
./parallel/prparallel "input_data/graph4.txt" "results/graph4_parallel_results.json"

./serial/prserial "input_data/graph5.txt" "results/graph5_serial_results.json"
./parallel/prparallel "input_data/graph5.txt" "results/graph5_parallel_results.json"

./serial/prserial "input_data/graph6.txt" "results/graph6_serial_results.json"
./parallel/prparallel "input_data/graph6.txt" "results/graph6_parallel_results.json"

./serial/prserial "input_data/graph7.txt" "results/graph7_serial_results.json"
./parallel/prparallel "input_data/graph7.txt" "results/graph7_parallel_results.json"

./serial/prserial "input_data/graph8.txt" "results/graph8_serial_results.json"
./parallel/prparallel "input_data/graph8.txt" "results/graph8_parallel_results.json"

./serial/prserial "input_data/graph9.txt" "results/graph9_serial_results.json"
./parallel/prparallel "input_data/graph9.txt" "results/graph9_parallel_results.json"

./serial/prserial "input_data/graph10.txt" "results/graph10_serial_results.json"
./parallel/prparallel "input_data/graph10.txt" "results/graph10_parallel_results.json"

./serial/prserial "input_data/graph11.txt" "results/graph11_serial_results.json"
./parallel/prparallel "input_data/graph11.txt" "results/graph11_parallel_results.json"

./serial/prserial "input_data/graph12.txt" "results/graph12_serial_results.json"
./parallel/prparallel "input_data/graph12.txt" "results/graph12_parallel_results.json"